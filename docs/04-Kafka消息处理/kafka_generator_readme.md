...
2. **自定义字段**（可选）：在自定义字段面板中覆盖特定字段值
   ...
   - 支持查看字段字典（枚举值说明）
   - 支持查看和选择历史输入值
   - **聚焦高亮提示**：输入框获得焦点时，字段卡片显示蓝色边框 + 发光效果，便于定位当前编辑位置
...
- `TIME_STAMP` 从 `EVENT_TIME` 转换为Unix时间戳字符串
...
#### 业务关键字段（4个）
这些字段强制使用代码默认规则，避免数据库配置错误导致数据异常：
- `CITY_ID`：城市ID，从 `_source.CITY_ID` 获取
- `BUSINESS_TYPE`：业务类型，从 `_source.BUSINESS_TAG.BUSINESS_TYPE` 获取
- `ACTIVE_STATUS`：激活状态，固定值为 `"1"`

### 字段类型规范
Kafka消费者对字段类型有严格要求，生成器会按照以下规则处理：

**数字类型字段（int）**：
- `STANDARD_FLAG` - 标准化标志
- `REDEFINE_SEVERITY` - 重定义告警级别
- `OBJECT_CLASS` - 设备类型ID

**字符串类型字段（str）**：
除上述3个字段外，所有其他字段均为字符串类型，包括数值型字段如：
- `ACTIVE_STATUS`、`VENDOR_ID`、`ALARM_RESOURCE_STATUS`
- `OBJECT_LEVEL`、`ORG_TYPE`、`SEND_JT_FLAG`
- `PREHANDLE`、`PORT_NUM`、`INTERFERENCE_FLAG`
- `ALARM_ABNORMAL_TYPE` 等

**注意**：即使某些字段值是数字（如 `VENDOR_ID: "8"`），也必须保持为字符串格式。
...
2. 特殊字段（时间、FP、业务关键字段）**忽略数据库配置**，强制使用生成逻辑
...
2. **灵活覆盖**：
   - 支持前端自定义任意字段值
   - 自定义值优先级最高，可覆盖自动生成值
   - **智能排序**：手动置顶的字段在最前，其他字段保持原始顺序
   - **视觉反馈**：置顶字段显示📌图标（带脉冲动画），有值字段显示✓标识
   - **聚焦高亮**：输入框获得焦点时，字段卡片显示蓝色边框 + 蓝色发光效果 + 蓝色背景，明确指示当前编辑位置
...
6. 自定义字段优先级高于自动映射值，但特殊字段（时间、FP、业务关键字段）会忽略自定义值
7. 字段类型严格遵循Kafka消费者要求：仅 `STANDARD_FLAG`、`REDEFINE_SEVERITY`、`OBJECT_CLASS` 为int类型，其他均为字符串

### 数据库配置
8. 数据库中的 `kafka_field_meta` 表可以动态配置字段映射，但**不会影响特殊字段**
9. 如果数据库连接失败，系统会自动降级使用内置的默认映射规则
10. 修改数据库配置后，刷新页面即可生效，无需重启服务

### 性能优化
11. 历史记录会自动分页加载，每页20条，避免一次性加载过多数据
12. 字段字典数据会在页面加载时一次性获取，减少重复请求
13. 大数据量的ES文档（>10MB）可能会导致解析缓慢，建议精简后再输入
...
**Q: 为什么某些字段值与预期不符？**  
A: 可能是数据库 `kafka_field_meta` 表配置错误。目前以下字段已强制使用代码默认规则，不受数据库影响：
   - 时间字段：EVENT_TIME、CREATION_EVENT_TIME、EVENT_ARRIVAL_TIME、TIME_STAMP
   - FP字段：FP0_FP1_FP2_FP3、CFP0_CFP1_CFP2_CFP3、ORIG_ALARM_FP、ORIG_ALARM_CLEAR_FP、SRC_ORG_ID
   - 业务字段：CITY_ID、BUSINESS_TYPE、ACTIVE_STATUS
