...
def ai_assisted_expand_function_points(original_points: list, expand_count: int,
                                       progress_callback=None, task_id: str = None,
                                       use_omlx: bool = False) -> list:
    """
    ...
    Args:
        ...
        task_id: 任务 ID
        use_omlx: 是否使用 OMLX 模型（Qwen3.5-4B-OptiQ-4bit），默认 False 使用本地 Ollama

    Returns:
        新生成的功能点列表（已去重）
    """
    from utils.ollama_client import get_ollama_client_for_fpa
    
    # 在调用 AI 前检查服务是否可用
    logger.info(f"[AI_EXPAND] 开始检查 AI 服务状态...")
    ollama = get_ollama_client_for_fpa()
    
    if ollama is None:
        logger.error(f"[AI_EXPAND] ❌ AI 服务不可用！请检查 OMLX 服务是否正常运行")
        logger.error(f"[AI_EXPAND]    - 如果服务未启动，请先启动 OMLX 服务")
        logger.error(f"[AI_EXPAND]    - 如果服务已启动，请检查网络连接和配置")
        # 抛出异常，让上层捕获并处理
        raise Exception("AI 服务不可用，请检查 OMLX 服务状态")
    
    logger.info(f"[AI_EXPAND] ✅ AI 服务检查通过，可以正常使用")

    # 记录使用的模型信息
    logger.info(f"[AI_EXPAND] 💡 客户端信息:")
    logger.info(f"[AI_EXPAND]    - API 地址：{ollama.base_url}")
    logger.info(f"[AI_EXPAND]    - 模型名称：{ollama.model}")
    logger.info(f"[AI_EXPAND]    - 使用模式：{'OMLX 在线模型' if ollama.use_omlx else '本地 Ollama 模型'}")
    ...
    max_workers = 2  # 单线程顺序请求，避免远程服务器过载
    ...
    logger.info(
        f"[AI_EXPAND] 处理 {len(sorted_to_split)} 个功能点，预计耗时：{len(sorted_to_split) * 8 / max_workers:.1f}秒（4 线程并发）")
    ...
    for i, point in enumerate(original_points):
        ...
        if i in expanded_map:
            for exp_point in expanded_map[i]:
                ...
                expanded_count += 1
                logger.info(
                    f"[AI_EXPAND] 插入扩展功能点到原始功能点 '{point.get('功能点计数项', '')}' 后面：{exp_point.get('功能点计数项', '')}")

    logger.info(f"[AI_EXPAND] 最终扩展功能点总数：{expanded_count}")
    logger.info(
        f"[AI_EXPAND] 插入位置验证：原始功能点数={len(original_points)}, 最终列表数={len(final_expanded_points)}")
